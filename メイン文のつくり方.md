...
コード全体は[こちら](https://github.com/ShoheiMiyata/phyvac/tree/main/MainSample/1AHP_simpleAHU)にアップロードされています。 
  
このプログラムで利用するモジュールをインポートします。
```
import phyvac as pv
import pandas as pd
import time
...
``` 
### 入力ファイルの読み込み
ここでは入力データが15分間隔のため、1分間隔で計算するために変換します。
```
...
流量バランスけいさんを行うためには、運転時に常に流量が生じる枝を設定する必要があります。これは配管網が複雑になっても同様です。  
