...
class AHU_simple:
    ...
    def cal(self, g, tin, q_load):
        self.g = g
        self.tin = tin
        self.q_load = q_load * 1000  # GJ -> MJ
        if self.g > 0:
            self.flag = 0
            self.tout = self.tin + self.q_load / 4.184 / self.g
            if self.tout > 25:
                self.tout = 25
                self.flag = 1
            self.q = (self.tout - self.tin) * self.g * 4.184 / 1000  # MJ -> GJ
...
class AirSourceHeatPump:
    ...
    def __init__(self, tin_ch_d=12, tout_ch_d=7, g_ch_d=0.116, kr_ch=13.9, signal_hp=1,
                 coef=[[40.0, -6.2468, 15.891, -14.257, 4.2438, 2.3663],
                       ...
                       [15.0, -6.2842, 17.284, -17.138, 5.228, 5.4071]]):  # 冷房時はこちら
        ...
        # db         :外気乾球温度[℃]
        # signal     :運転信号(0=stop, 1=cooling, 2=heating)
    ...
    def cal(self, tin_ch, g_ch, tdb, signal_hp):
        self.tin_ch = tin_ch
        self.g_ch = g_ch
        self.tdb = tdb
        self.signal_hp = signal_hp
        ...
        if self.signal_hp == 1:
            if (self.g_ch > 0) and (self.tin_ch > self.tout_ch_d):
                ...
                # self.pl  : 0.2以下 or 1.0以上になり得る、処理熱量/定格処理熱量によって計算した負荷率
                # pl       : 0.2以上 1.0以下 に収まる、COP計算用の運転時負荷率
                if self.pl > 1:
                    self.pl = 1  # 実際の負荷率も1.0以上になることはないと定義
                    pl = self.pl
                ...
                if self.tdb >= self.coef[0][0]:
                    ...
                    for i in range(1, self.n):  # 線形補間の上限・下限となる曲線を探す
                        ...
                        if self.coef_b[0] <= self.tdb < self.coef_a[0]:
                            break
                    ...
                    self.COP = (a - b) * (self.coef_a[0] - self.tdb) / (self.coef_a[0] - self.coef_b[0]) + b
                # 消費電力の計算
        ...
        if self.signal_hp == 2:
            if (self.g_ch > 0) and (self.tin_ch < self.tout_ch_d):
                self.tout_ch = self.tout_ch_d
                pl = (self.tout_ch - self.tin_ch) * self.g_ch / (self.max_del_t * self.g_ch_d)
                # self.pl  : 0.2以下 or 1.0以上になり得る、処理熱量/定格処理熱量によって計算した負荷率
                # pl       : 0.2以上 1.0以下 に収まる、COP計算用の運転時負荷率
                if self.pl > 1:
                    self.pl = 1
