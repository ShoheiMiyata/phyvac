...
plt.plot(x, y)
plt.title('Fan P-Q curve')
plt.xlabel('flow [m3/min]')
...
for i in range(1000):
    ...
    g_ab_eva = 20.0
    cnt1 = 0
    while(g_ab_eva > 0.01)or(g_ab_eva < -0.01):
        cnt1 += 1
        ...
        g_eb_eva = 10
        cnt2 = 0
        while(g_eb_eva > 0.01)or(g_eb_eva < -0.01):
            cnt2 += 1
    ...
    print(Fan_SA.inv,Fan_RA.inv,Fan_EA.inv,Branch_ab.kr_duct,Branch_ef.kr_duct)
