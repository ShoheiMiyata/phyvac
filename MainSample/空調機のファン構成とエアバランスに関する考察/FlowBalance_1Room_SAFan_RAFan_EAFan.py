...
# 機器の定義
Fan_SA = pv.Fan(pg=[100, -0.5, -1.0])
Fan_RA = pv.Fan(pg=[100, -0.5, -1.0])
Fan_EA = pv.Fan(pg=[100, -0.5, -1.0])
...
Fan_RA.inv = 0.7
Fan_EA.inv = 0.4
Branch_ab.kr_duct = 1.5
Branch_bc.kr_duct = 1.5
Branch_gh.kr_duct = 1.5
Branch_de.kr_duct = 1.5
Branch_eb.kr_duct = 1.5
Branch_ef.kr_duct = 1.5
...
while(g_ab_eva > 0.001)or(g_ab_eva < -0.001):
    ...
    while(g_eb_eva > 0.001)or(g_eb_eva < -0.001):
        ...
        g_gh = Branch_gh.p2f(-dp_eb-dp_bc)
        g_de = g_bc - g_gh
...
print(cnt1, cnt2)
print(round(dp_ab+dp_bc,3))
print("")
print(round(Branch_ef.g,2), "  ",  round(Branch_de.g,2))
print("    ",round(Branch_eb.g,2), "  ", round(Branch_gh.g,2))
