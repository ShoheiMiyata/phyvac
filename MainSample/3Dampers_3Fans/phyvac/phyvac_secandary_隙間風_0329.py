...
# ダンパ特性と圧力損失計算
class Damper:
    def __init__(self, coef=[[1.0, -0.00001944, 0.018, 0.18, -0.007], [0.8, 0.0000864, 0.036, 0.132, 0.0684],
                  [0.6, 0.001296, 0.072, 0.384, 0.1001], [0.4, 0.00108, 0.36, -0.582, 0.0662],
                  [0.2, -0.0216, 4.32, -5.34, 0.2527]]):
        # damp  :ダンパ開度[0.0~1.0]
        # dp    :圧力損失[kPa]  # 単位注意
        # g     :流量[m^3/min]
        # dp = a * g^3 + b * g^2 + c * g + d
        # coef = [[damp1, a_damp1, b_damp1, c_damp1, d_damp1], ... , [dampn, a_dampn, b_dampn, c_dampn, d_dampn]]
        # (x1 >...> xn, n>=3)
        self.coef = coef
        self.g = 0
        self.damp = 0
    ...
    def loss(self, g, damp):
        self.g = g
        self.damp = damp
        n = len(self.coef)
        if self.damp >= self.coef[0][0]:
            self.dp = self.coef[0][1] * self.g ** 2
        elif self.damp < self.coef[n - 1][0]:
            self.dp = self.coef[n - 1][1] * self.g ** 2
        else:
        ...
        return self.dp * (-1000)  # 圧力損失をPaで返す

# ファン特性と消費電力計算
class Fan:
    # 定格値の入力
    def __init__(self, pg=[0.6467, 0.0082, -0.0004, 0, 0], eg=[-0.0166, 0.0399, -0.0008], r_ef=0.8, inv=1.0):
        # pg    :圧力-流量(pg)曲線の係数（切片、一次、二次）or （切片、一次、二次、三次、四次）
    ...
    def f2p(self, g):  # flow to pressure for fan
        ...
        if self.g > 0:
            # self.dp = (self.pg[0] + self.pg[1] * (self.g / self.inv) + self.pg[2] * (
            #         self.g / self.inv) ** 2) * self.inv ** 2
            self.dp = (self.pg[4] * (self.g / self.inv) ** 4 + self.pg[3] * (self.g / self.inv) ** 3 + self.pg[2] *
                       (self.g / self.inv) ** 2 + self.pg[1] * (self.g / self.inv) + self.pg[0]) * self.inv ** 2
        else:
            self.dp = 0
...
class Branch11:  # コンポジションというpython文法を使う
    ...
    def f2p(self, g):  # 流量から圧力損失を求める
        ...
        if self.pump.inv == 0 and self.valve.vlv > 0:
            ...
            [co_0c, co_1c, co_2c] = [co_0a + co_2b * self.g ** 2, co_1a - co_2b * 2 * self.pump.num * self.g,
                                     co_2a + co_2b * self.pump.num ** 2]
...
class Branch30:
    ...
    def f2p(self, g):  # 流量から圧力損失を求める flow to pressure for branch
        self.g = g
        self.dp = self.s * self.g ** 2
        return self.dp  # Pa

    def p2f(self, dp):  # 圧力損失から流量を求める
        self.dp = dp
        self.g = (self.dp / self.s) ** 0.5
        return self.g
